...
MasterSchedule MasterSchedule::GetSchedule(MasterConfig aCfg, Master* apMaster, AsyncTaskGroup* apGroup)
{
	AsyncTaskBase* pIntegrity = apGroup->Add(aCfg.IntegrityRate, aCfg.TaskRetryRate, AMP_POLL, bind(&Master::IntegrityPoll, apMaster, _1), "Integrity Poll");	
	...
	if(aCfg.DoUnsolOnStartup)
	{
		...
		TaskHandler handler = bind(&Master::ChangeUnsol, apMaster, _1, false, PC_ALL_EVENTS);
		...
		if(aCfg.EnableUnsol)
		{
			TaskHandler handler = bind(&Master::ChangeUnsol, apMaster, _1, true, aCfg.UnsolClassMask);
	...
	{
		AsyncTaskBase* pEventScan = apGroup->Add(e.ScanRate, aCfg.TaskRetryRate, AMP_POLL, bind(&Master::EventPoll, apMaster, _1, e.ClassMask), "Event Scan");
	...
	schedule.mpCommandTask = apGroup->AddContinuous(AMP_COMMAND, boost::bind(&Master::ProcessCommand, apMaster, _1), "Command");
	schedule.mpTimeTask = apGroup->AddContinuous(AMP_TIME_SYNC, boost::bind(&Master::SyncTime, apMaster, _1), "TimeSync");	
	schedule.mpClearRestartTask = apGroup->AddContinuous(AMP_CLEAR_RESTART, boost::bind(&Master::WriteIIN, apMaster, _1), "Clear IIN");
