	...
	*/
	class Database : public IDataObserver, public Loggable
	{
		public:
			...
			void Begin(BinaryIterator& arIter)		{ arIter = mBinaryVec.begin(); }
			void Begin(AnalogIterator& arIter)		{ arIter = mAnalogVec.begin(); }
	...
	size_t Database::CalcNumType(const std::vector<T*>& arIdxVec)
	{
		...
		for(size_t i=0; i<arIdxVec.size(); ++i) { indexSet.insert(arIdxVec[i]->Index); }
