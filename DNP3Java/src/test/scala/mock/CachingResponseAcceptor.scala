...
class CachingResponseAcceptor extends IResponseAcceptor {
  ...
  val responses = new SynchronizedList[Response]
  case class Response(response: CommandResponse, sequence: Int)

  override def AcceptResponse(response: CommandResponse, sequence: Int) = responses.append(Response(response, sequence))
