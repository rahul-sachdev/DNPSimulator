...
BOOST_AUTO_TEST_SUITE(AsyncTransportSuite)
	...
	BOOST_AUTO_TEST_CASE(TestReceiveLargestPossibleAPDU)
	{
		...
		BOOST_FOREACH(string s, packets) { test.lower.SendUp(s); }

		BOOST_REQUIRE(test.IsLogErrorFree());
		BOOST_REQUIRE(test.upper.BufferEquals(apdu)); //check that the correct data was written
	}
	...
	BOOST_AUTO_TEST_CASE(TestSendFullAPDU)
	{
		...
		{
			BOOST_REQUIRE(test.lower.BufferEquals(tpdu));
