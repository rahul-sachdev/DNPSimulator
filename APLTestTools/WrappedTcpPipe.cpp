...
WrappedTcpPipe::WrappedTcpPipe(Logger* apLogger, boost::asio::io_service* apService, boost::uint16_t aPort) :
	clientTcp(apLogger->GetSubLogger("clientTcp"), apService, TcpSettings("127.0.0.1", aPort)),
	serverTcp(apLogger->GetSubLogger("serverTcp"), apService, TcpSettings("127.0.0.1", aPort)),
